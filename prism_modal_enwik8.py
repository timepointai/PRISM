"""
prism_modal_enwik8.py — the enwik8 probe — dirs_only on the canonical byte-LM bench, `exp/enwik8`
branch. One isolated Volume, one detached L4 job:

    modal run --detach prism_modal_enwik8.py
...
Fetch + COMMIT the artifact:
    modal volume get prism-eval-enwik8 nanogpt-prism/results/finetune_<stamp>.json ./results/
"""
import modal

app = modal.App("prism-eval-enwik8")
...
vol = modal.Volume.from_name("prism-eval-enwik8", create_if_missing=True)
WORK = "/work"
REPO = f"{WORK}/nanogpt-prism"
REPO_URL = "https://github.com/timepointai/PRISM.git"
BRANCH = "exp/enwik8"

FLAGS = ("--corpus=enwik8 --method=dirs_only --seeds=1337,1338,1339 "
         "--teacher_steps=2000 --student_steps=1500 --eval_every=10 "
         "--eval_iters=50 --method_lr=1e-3 --method_warmup=100")
...
def main(gpu: str = "L4", extra: str = ""):
    ...
    print("artifact lands on Volume prism-eval-enwik8 under nanogpt-prism/results/")
