"""
prism_modal_catalog.py — the catalog-task retention-dial probe, on the `exp/catalog-task`
branch. One isolated Volume, one detached L4 job:

    modal run --detach prism_modal_catalog.py
...
Fetch + COMMIT the artifact:
    modal volume get prism-eval-catalog nanogpt-prism/results/finetune_<stamp>.json ./results/
"""
import modal

app = modal.App("prism-eval-catalog")
...
vol = modal.Volume.from_name("prism-eval-catalog", create_if_missing=True)
WORK = "/work"
REPO = f"{WORK}/nanogpt-prism"
REPO_URL = "https://github.com/timepointai/PRISM.git"
BRANCH = "exp/catalog-task"

FLAGS = ("--old_corpus=modernweb --far_corpus=data/catalog/task_train.txt "
         "--ft_val_corpus=data/catalog/task_val.txt "
         "--recall_prompts=data/catalog/recall_prompts.tsv "
         ...
         "--arms=base,plain,raw_weak,raw_mid,lowlr_b --tag=catalog1")
...
def main(gpu: str = "L4", extra: str = ""):
    ...
    print("artifact lands on Volume prism-eval-catalog under nanogpt-prism/results/")
